            ...
            <div className="space-y-2 text-sm leading-snug">
              {data.personal_info?.phone && (
                <div className="flex items-center gap-2">
                  <Phone size={15} color={accentColor} className="shrink-0" />
                  <span className="break-all">{data.personal_info.phone}</span>
                </div>
              )}
              {data.personal_info?.email && (
                <div className="flex items-center gap-2">
                  <Mail size={15} color={accentColor} className="shrink-0" />
                  <span className="break-all">{data.personal_info.email}</span>
                </div>
              )}
              {data.personal_info?.location && (
                <div className="flex items-center gap-2">
                  <MapPin size={15} color={accentColor} className="shrink-0" />
        ...
        <main className="col-span-2 p-8 pt-0">
          ...
          {data.professional_summary && (
            <section className="mb-8">
              ...
              <p className="text-zinc-700 text-justify leading-snug whitespace-pre-wrap break-words overflow-x-hidden">
                {data.professional_summary}
              </p>
            </section>
          )}
              ...
              <div className="space-y-6 mb-8">
                {data.experience.map((exp, index) => (
                  <div key={index}>
                    ...
                    {exp.description && (
                      <ul className="list-disc list-inside text-justify text-sm text-zinc-700 leading-snug space-y-1 break-all whitespace-pre-wrap overflow-x-hidden">
            ...
            <section>
              ...
              <div className="space-y-4">
                {data.project.map((project, index) => (
                  <div key={index}>
                    <h3 className="text-md font-bold text-zinc-800 mt-3">
                      {project.name}
                    </h3>
                    <p className="text-sm mb-1" style={{ color: accentColor }}>
                      {project.type}
